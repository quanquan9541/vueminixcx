        ...
        <uni-table ref="table" :loading="loading" :emptyText="error.message || '没有更多数据'" border stripe type="selection"
          ...
          <uni-tr>
            ...
            <uni-th align="center" filter-type="search" @filter-change="filterChange($event, 'goods_desc')" sortable
              @sort-change="sortChange($event, 'goods_desc')">配料</uni-th>
            <uni-th align="center" filter-type="search" @filter-change="filterChange($event, 'url')" sortable
              @sort-change="sortChange($event, 'url')">图文</uni-th>
            <uni-th align="center" filter-type="timestamp" @filter-change="filterChange($event, 'add_date')" sortable
              @sort-change="sortChange($event, 'add_date')">日期</uni-th>
...
<script>
  ...
  export default {
    data() {
      return {
        collectionList: [db.collection('tea-milk-list').field('category_id,name,pic,goods_desc,url,add_date,state')
          .getTemp(), db.collection('tea-milk-class').field('_id, name as text').getTemp()
        ],
        query: '',
        where: '',
        orderby: dbOrderBy,
